...
// ─── GET EMPLOYEE POLICY ─────────────────────────────────────
export const getEmployeePolicy = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
  ...
  try {
    ...
    if (geofenceOverride === 'custom' && customGeofenceIds?.length > 0) {
      const gfCheck = await pool.query(
        'SELECT id FROM geofence_locations WHERE id = ANY($1) AND is_active = true',
        [customGeofenceIds]
      );
...
export const getAllGeofences = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const result = await pool.query(
      `SELECT id, name, latitude, longitude, radius_meters, address, branch_code, is_active
...
export const createGeofence = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO geofence_locations (name, latitude, longitude, radius_meters, address, branch_code, is_active, created_by)
...
export const resetToDefault = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { userId } = req.params;
    const targetId = parseInt(userId);
