 ...
 */
async function createSuperAdmin(client, data) {
    try {
        // Check if admin already exists
        const existingAdmin = await client.query('SELECT id, email, employee_id FROM users WHERE email = $1 OR employee_id = $2', [data.email, 'SA001']);
 ...
 */
async function createAttendancePolicy(client, userId) {
    try {
    ...
    `);
      ...
      `);
        ...
        const existingPolicy = await client.query('SELECT id FROM attendance_policies WHERE user_id = $1', [userId]);
        ...
        const result = await client.query(`INSERT INTO attendance_policies (
        user_id,
        geofence_override,
        photo_required,
        allow_remote
      ) VALUES ($1, $2, $3, $4)
      RETURNING id`, [userId, 'remote', false, true]);
        return {
            success: true,
 ...
 */
async function createGeofence(client, data) {
    try {
    ...
    `);
        ...
        if (columnsExist.rows.length < 2) {
            const hasbranchCode = columnsExist.rows.some((r) => r.column_name === 'branch_code');
            const hasAddress = columnsExist.rows.some((r) => r.column_name === 'address');
            if (!hasbranchCode) {
                await client.query(`
        ...
        const existingGeofence = await client.query('SELECT id FROM geofence_locations WHERE branch_code = $1', ['HQ001']);
        if (existingGeofence.rows.length > 0) {
            // Update existing geofence
            await client.query(`UPDATE geofence_locations
         SET name = $1,
             latitude = $2,
             longitude = $3,
             radius_meters = $4,
        ...
        // Create new geofence
        const result = await client.query(`INSERT INTO geofence_locations (
        name,
        latitude,
        longitude,
        radius_meters,
